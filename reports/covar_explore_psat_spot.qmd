...
cmem_dat60 %>% 
  filter(PA == 0) %>%
  ggplot(aes(x = lat, y = pO2_atm_0m, fill = O2_demand_60m)) + 
  geom_point(shape = 21, size = 3)+
  theme(legend.position = "right")+
  scale_fill_viridis_c()
...
AGI_surf <- expand_grid(o2 = o2_range, temp = temp_range) %>% 
  mutate(met_dem = OxyDemand(Tpref = 16.452, PO2_thresh = 0.0488, T_C = temp), 
         AGI = o2/met_dem)
...
cmem_dat60 %>% 
  filter(PA == 0) %>%
  ggplot(aes(x = bathy_mean, y = AGI_60m, color = as.factor(PA))) + 
  geom_point(alpha = 0.2) +
  scale_color_manual(values = c("#0a3351"))+
  theme(legend.position = "none")
```
...
temp_range <- seq(6, 30, length.out = 100)
AGI_surf <- expand_grid(o2 = o2_range, temp = temp_range) %>% 
  mutate(met_dem = OxyDemand(Tpref = 16.452, PO2_thresh = 0.046413, T_C = temp), 
         AGI = o2/met_dem)
...
cmem_dat250 %>% 
  filter(PA == 0) %>%
  ggplot(aes(x = bathy_mean, y = AGI_250m, color = as.factor(PA))) + 
  geom_point(alpha = 0.2) +
  scale_color_manual(values = c("#0a3351"))+
  theme(legend.position = "none")
```
...
temp_range <- seq(6, 30, length.out = 100)
AGI_surf <- expand_grid(o2 = o2_range, temp = temp_range) %>% 
  mutate(met_dem = OxyDemand(Tpref = 16.452, PO2_thresh = 0.03815, T_C = temp), 
         AGI = o2/met_dem)
