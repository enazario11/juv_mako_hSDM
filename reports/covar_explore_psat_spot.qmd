...
cmem_dat <- readRDS(here("data/locs_w_covar/cmems/cmem_covar_AGI_atm_0m_emp.rds"))
...
cmem_dat %>% 
  filter(PA == 0) %>%
  ggplot(aes(x = thetao_mean, y = O2_demand0, color = as.factor(PA))) + 
  geom_point(alpha = 0.2) +
  scale_color_manual(values = c("#0a3351"))+
  theme(legend.position = "none")
```
...
cmem_dat %>% 
  filter(PA == 0) %>%
  ggplot(aes(x = thetao_mean, y = pO2_0_atm, fill = O2_demand0)) + 
...
cmem_dat %>% 
  filter(PA == 0) %>%
  ggplot(aes(x = lat, y = thetao_mean, fill = O2_demand0)) + 
  geom_point(shape = 21, size = 3)+
  theme(legend.position = "right")+
  scale_fill_viridis_c()
...
cmem_dat %>% 
  filter(PA == 0) %>%
  ggplot(aes(x = lat, y = pO2_0_atm, fill = O2_demand0)) + 
  geom_point(shape = 21, size = 3)+
  theme(legend.position = "right")+
  scale_fill_viridis_c()
...
cmem_dat %>% 
  filter(PA == 0) %>%
  ggplot(aes(x = lon, y = thetao_mean, fill = O2_demand0)) + 
  geom_point(shape = 21, size = 3)+
  theme(legend.position = "right")+
  scale_fill_viridis_c()
...
cmem_dat %>% 
  filter(PA == 0) %>%
  ggplot(aes(x = lon, y = pO2_0_atm, fill = O2_demand0)) + 
  geom_point(shape = 21, size = 3)+
  theme(legend.position = "right")+
  scale_fill_viridis_c()
...
o2_range <- seq(198.6259, 324.4232, length.out = 100)
