...
cmem_dat %>% 
  filter(PA == 0) %>%
  ggplot(aes(x = thetao_mean, y = O2_demand_mako250, color = as.factor(PA))) + 
  geom_point(alpha = 0.2) +
  scale_color_manual(values = c("#0a3351"))+
  theme(legend.position = "none")
```
...
cmem_dat %>% 
  filter(PA == 0) %>%
  ggplot(aes(x = thetao_mean, y = pO2_250_atm, fill = O2_demand_mako250)) + 
...
cmem_dat %>% 
  filter(PA == 0) %>%
  ggplot(aes(x = lat, y = thetao_mean, fill = O2_demand_mako250)) + 
  geom_point(shape = 21, size = 3)+
  theme(legend.position = "right")+
  scale_fill_viridis_c()
...
cmem_dat %>% 
  filter(PA == 0) %>%
  ggplot(aes(x = lat, y = pO2_250_atm, fill = O2_demand_mako250)) + 
  geom_point(shape = 21, size = 3)+
  theme(legend.position = "right")+
  scale_fill_viridis_c()
...
cmem_dat %>% 
  filter(PA == 0) %>%
  ggplot(aes(x = lon, y = thetao_mean, fill = O2_demand_mako250)) + 
  geom_point(shape = 21, size = 3)+
  theme(legend.position = "right")+
  scale_fill_viridis_c()
...
cmem_dat %>% 
  filter(PA == 0) %>%
  ggplot(aes(x = lon, y = pO2_250_atm, fill = O2_demand_mako250)) + 
  geom_point(shape = 21, size = 3)+
  theme(legend.position = "right")+
  scale_fill_viridis_c()
...
o2_range <- seq(0.00045, 0.1124, length.out = 100)
