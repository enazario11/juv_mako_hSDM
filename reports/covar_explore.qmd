...
rmarkdown::paged_table(covar_just)
```
...
cmem_dat %>% 
  filter(PA == 0) %>%
  ggplot(aes(x = thetao_mean, y = o2_mean, fill = AGI_0m)) + 
...
cmem_dat %>% 
  filter(PA == 0) %>%
  ggplot(aes(x = lat, y = thetao_mean, fill = o2_mean)) + 
  geom_point(shape = 21, size = 3)+
  theme(legend.position = "right")+
  scale_fill_viridis_c()
```
...
cmem_dat %>% 
  filter(PA == 0) %>%
  ggplot(aes(x = lon, y = thetao_mean, fill = o2_mean)) + 
  geom_point(shape = 21, size = 3)+
  theme(legend.position = "right")+
  scale_fill_viridis_c()
```
