...
sum_scatter <- ggplot(output_sum, aes(AUC, TSS, color = deviance_exp, label = model)) + 
  geom_point(size = 3) +
  xlab('AUC') +
  ylab('TSS') +
...
plotly::ggplotly(sum_scatter, tooltip = "label")
```
...
sum_scatter_Ntag <- ggplot(output_sum_Ntag, aes(AUC, TSS, color = deviance_exp, label = model)) + 
  geom_point(size = 3) +
  xlab('AUC') +
  ylab('TSS') +
  scale_color_gradientn(colors = MetBrewer::met.brewer("Greek")) +
...
plotly::ggplotly(sum_scatter_Ntag, tooltip = "label")
```
...
sum_scatter_seas_ann <- ggplot(output_sum_seas_ann, aes(AUC, TSS, color = deviance_exp, label = model)) + 
  geom_point(size = 3) +
  xlab('AUC') +
  ylab('TSS') +
  scale_color_gradientn(colors = MetBrewer::met.brewer("Greek")) +
...
plotly::ggplotly(sum_scatter_seas_ann, tooltip = "label")
```
