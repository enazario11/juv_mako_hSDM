...
lessR::PieChart(x = Predictor_variable, y = relative_influence, data = base_inf, fill = base_cols, main = NULL, quiet = TRUE, cex = 2, labes_size = 1.5) 
...
lessR::PieChart(x = Predictor_variable, y = relative_influence, data = do_inf, fill = do_cols, main = NULL, quiet = TRUE, cex = 2, labes_size = 1.5) 
...
lessR::PieChart(x = Predictor_variable, y = relative_influence, data = agi_inf, fill = agi_cols, main = NULL, quiet= TRUE, cex = 2, labes_size = 1.5) 
...
mod_metrics <- output_sum %>% filter(model == "base_Nwind_crw" | model == "agi_0m_250m_dail_seas_ann_crw" | model == "do_0m_250m_dail_seas_ann_crw") %>% mutate(model = as.factor(model), model = fct_relevel(model, c("base_Nwind_crw", "do_0m_250m_dail_seas_ann_crw", "agi_0m_250m_dail_seas_ann_crw")))
...
AUC <- ggplot(mod_metrics, aes(x = model, y=AUC)) +
  geom_segment(aes(x=model, xend=model, y=0, yend=AUC), color="darkslategrey", linewidth = 1.5) +
  geom_point(color="orange", size=8) +
  theme_light() +
  theme(
    panel.grid.major.x = element_blank(),
